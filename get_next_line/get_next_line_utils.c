...
/*   Updated: 2025/01/14 15:48:22 by imugica-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	next_ft_strlen(const char *c)
{
	int	i;
...
char	*next_ft_strjoin(char **s1, char const *s2, int bytes)
{
	size_t	lens1;
	size_t	lens2;
	size_t	lensdst;
	...
	if (!s2)
		return (0);
	if (*s1)
		lens1 = next_ft_strlen(*s1);
	else
		lens1 = 0;
	lens2 = next_ft_strlen(s2);
